...
def check_robots_txt(url: str) -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Check for exact "Disallow: /" pattern
            # (not "Disallow: /admin" etc.)
            # Look for "disallow: /" followed by whitespace or newline
            if "user-agent: *" in robots_content:
                lines = robots_content.split('\n')
                for line in lines:
                    line = line.strip()
                    # Check if line is exactly "disallow: /"
                    # or "disallow: /" followed by whitespace
                    if line.startswith("disallow:"):
                        ...
                        if path == "/":
                            return {
                                "allowed": False,
                                "warning": (
                                    "This website's robots.txt discourages "
                                    "scraping. Please respect their wishes."
                                ),
                            }
        return {"allowed": True}
    except Exception:
        ...
        try:
            ...
            for a in soup.find_all("a", href=True):
                ...
                if (p.scheme in ("http", "https")
                        and p.netloc == main_netloc
                        and abs_url != url
                        and abs_url not in seen):
                    seen.add(abs_url)
                    candidates.append(abs_url)

            for sub_url in candidates[:_MAX_SUBPAGES]:
                if not validate_url(sub_url)["valid"]:
                    sub_errors.append(sub_url)
                    continue
                try:
                    sub_resp = requests.get(sub_url, headers=headers, timeout=15)
                    sub_resp.raise_for_status()
                    sub_soup = BeautifulSoup(sub_resp.text, "html.parser")
                    for el in sub_soup(["script", "style", "nav", "footer", "header", "aside", "iframe"]):
                        el.decompose()
                    sub_main = sub_soup.find("main") or sub_soup.find("article") or sub_soup.find("body")
                    sub_text = sub_main.get_text(separator=" ", strip=True) if sub_main else sub_soup.get_text(separator=" ", strip=True)
                    sub_text = " ".join(sub_text.split())[:MAX_PAGE_CHARS]
                    sub_pages.append((sub_url, sub_text))
                    logger.log_event("SUBPAGE_SCRAPE_SUCCESS", {"url": sub_url})
                except Exception as e:
                    sub_errors.append(sub_url)
                    logger.log_error("SUBPAGE_SCRAPE_ERROR", str(e), {"url": sub_url})
        except Exception as e:
            logger.log_error("LINK_EXTRACTION_ERROR", str(e), {"url": url})

        combined = main_text + "".join(f"\n\n--- Page: {u} ---\n\n{t}" for u, t in sub_pages)

        existing_warning = robots_check.get("warning")
        error_warning = f"Could not scrape {len(sub_errors)} sub-page(s)." if sub_errors else None
        final_warning = " | ".join(filter(None, [existing_warning, error_warning])) or None

        logger.log_website_scraping(url, len(combined), success=True)
        return {
            "success": True,
            "title": title_text,
            "content": combined,
